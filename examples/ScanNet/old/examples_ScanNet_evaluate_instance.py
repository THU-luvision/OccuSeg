...
#        print(instance_flag.sum().item())
        ...
        if config['visualize_part']:
            ...
            pcd_center.points = open3d.Vector3dVector(xyz)
            ...
            pcd_center.colors = open3d.Vector3dVector(cm.hot(exsiting_offsets_device.data.cpu().numpy())[:,0,0:3])


            pcd_gt = open3d.geometry.PointCloud()
            pcd_gt.points = open3d.Vector3dVector(xyz + shiftX)
            gt_color = torch.zeros([xyz.shape[0],3],dtype=torch.float32)
            gt_color[point_instance_flag,1] = 1
            pcd_gt.colors = open3d.Vector3dVector(gt_color.data.numpy())

            pcd_pred_instance = open3d.geometry.PointCloud()
            pcd_pred_instance.points = open3d.Vector3dVector(xyz + shiftX*2)
            gt_color = cm.hot(torch.gather(prob, dim = 0, index = region_mask).data.cpu().numpy())[:,0:3]
            pcd_pred_instance.colors = open3d.Vector3dVector(gt_color)
            ...
            pcd_prob1.points = open3d.Vector3dVector(xyz + shiftX*3)
            gt_color = cm.hot(torch.gather(p1, dim = 0, index = region_mask).data.cpu().numpy())[:,0:3]
            pcd_prob1.colors = open3d.Vector3dVector(gt_color)

            pcd_prob2 = open3d.geometry.PointCloud()
            pcd_prob2.points = open3d.Vector3dVector(xyz + shiftX*4)
            gt_color = cm.hot(torch.gather(p2, dim = 0, index = region_mask).data.cpu().numpy())[:,0:3]
            pcd_prob2.colors = open3d.Vector3dVector(gt_color)
    ...
    #            print(k,mean_shift_vector.norm(dim = 0).item(), instance_flag.sum(0).item())
        ...
        if False:
            ...
            for k in range(torch.max(detailed_instances)+1):
                ...
                """
                pcd_gt = open3d.geometry.PointCloud()
                pcd_gt.points = open3d.Vector3dVector(pose_device[instance_flag,:][detailed_instances==k,:].cpu().numpy())
                pcd_gt.colors = open3d.Vector3dVector(np.random.rand(1,3).repeat((detailed_instances==k).sum(0),axis=0))
                pcds.append(pcd_gt)
                """
...
#    pdb.set_trace()
    ...
    while instance_count <  expected_instance_num and background_mask.sum(0) > minimum_instance_size * 0.5:
        ...
        if config['visualize_part']:
            ...
            pcd_semantic.points = open3d.Vector3dVector(xyz + shiftX*5)
            pcd_semantic.colors  = open3d.Vector3dVector(label2color(pred_semantic))
...
#            open3d.visualization.draw_geometries([pcd_center, pcd_gt,pcd_pred_instance,pcd_prob1,pcd_prob2])
    ...
    if False:
        ...
        pcd_gt = open3d.geometry.PointCloud()
        pcd_gt.points = open3d.Vector3dVector(xyz + shiftX)
        pcd_gt.colors = open3d.Vector3dVector(gt_colors)

        pcd_joint_pred = open3d.geometry.PointCloud()
        pcd_joint_pred.points = open3d.Vector3dVector(xyz + shiftX * 2)
        pcd_joint_pred.colors = open3d.Vector3dVector(joint_pred_colors)

        pcd_ori = open3d.geometry.PointCloud()
        pcd_ori.points = open3d.Vector3dVector(xyz + shiftX * 3)
        pcd_ori.colors = open3d.Vector3dVector(original_colors)
...
#                if (occupancy_ratio > config['occupancy_ratio_threshold']) and (occupancy_ratio < config['occupancy_ratio_threshold_up']):
    ...
    #        proposals[i] = new_proposals
        ...
        if config['visualize']:
            pcd_gt = open3d.geometry.PointCloud()
            pcd_gt.points = open3d.Vector3dVector(xyz+ shiftX * 2)
            ...
            pcd_gt.colors = open3d.Vector3dVector(gt_color)



            pcd_occupancy_instance = open3d.geometry.PointCloud()
            pcd_occupancy_instance.points = open3d.Vector3dVector(xyz + shiftX * 2 + shiftY)
            ...
            pcd_occupancy_instance.colors = open3d.Vector3dVector(gt_color)

            pcd_leak_instance = open3d.geometry.PointCloud()
            pcd_leak_instance.points = open3d.Vector3dVector(xyz + shiftX + shiftY)
            ...
            pcd_leak_instance.colors = open3d.Vector3dVector(gt_color)


            pcd_pred_instance = open3d.geometry.PointCloud()
            pcd_pred_instance.points = open3d.Vector3dVector(xyz)
            random_colors = np.random.rand(np.unique(pred_instance).size,3)
            mask = np.unique(pred_instance ,False,True)[1]
            gt_color = random_colors[mask,:]
#            gt_color[:, :] = 0
            ...
            #open3d.Vector3dVector(pred_displacements + 0.5)
            #open3d.Vector3dVector(cm.hot(pred_offsets)[:,0,0:3])#
            #open3d.Vector3dVector(gt_color)#

            """
            pcd_color = open3d.geometry.PointCloud()
            pcd_color.points = open3d.Vector3dVector(xyz+ shiftX * 3)
            random_colors = np.random.rand(np.unique(pred_instance).size,3)
            mask = np.unique(pred_instance ,False,True)[1]
            gt_color = random_colors[mask,:]
            pcd_color.colors = open3d.Vector3dVector((1 + features[:,0:3]) / 2)

            open3d.visualization.draw_geometries([pcd_pred_instance,pcd_gt,pcd_color,pcd_leak_instance,pcd_occupancy_instance])
        """
        if True:
            pcd_pred_instance = open3d.geometry.PointCloud()
            pcd_pred_instance .points = open3d.Vector3dVector(xyz + shiftX)
            ...
            pcd_gt = open3d.geometry.PointCloud()
            pcd_gt.points = open3d.Vector3dVector(xyz)
            random_colors = np.random.rand(np.unique(true_instance).size,3)
            mask = np.unique(true_instance,False,True)[1]
            gt_color = random_colors[mask,:]
            pcd_gt.colors = open3d.Vector3dVector(gt_color)
            ...
            pcd_ori = open3d.geometry.PointCloud()
            pcd_ori.points = open3d.Vector3dVector(xyz + shiftY)
            pcd_ori.colors = open3d.Vector3dVector(cm.hot(pred_offsets)[:,0,0:3])


            pcd_gt_hot = open3d.geometry.PointCloud()
            pcd_gt_hot.points = open3d.Vector3dVector(xyz + shiftX + shiftY)
            pcd_gt_hot.colors = open3d.Vector3dVector(cm.hot(true_offsets)[:,0,0:3])

#            pcd_ori.colors = open3d.Vector3dVector(cm.hot(pred_offsets)[:,0,0:3])

        else:
            pcd_gt = open3d.geometry.PointCloud()
            pcd_gt.points = open3d.Vector3dVector(xyz + shift)
            ...
            pcd_ori = open3d.geometry.PointCloud()
            pcd_ori.points = open3d.Vector3dVector(xyz)
            pcd_ori.colors = open3d.Vector3dVector(visualization)
            print(xyz.shape, visualization.shape)

            pcd_color = open3d.geometry.PointCloud()
            pcd_color.points = open3d.Vector3dVector(xyz + shift*4)
            pcd_color.colors = open3d.Vector3dVector((1 + features[:,0:3]) / 2)
...
#        open3d.visualization.draw_geometries([pcd_pred_instance, pcd_gt, pcd_gt_hot, pcd_ori])
    ...
    os.system('python ~/hdd10T/projects/MASC/scripts/evaluate_semantic_instance.py')
