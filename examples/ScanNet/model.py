...
class ClusterSegNet(nn.Module):
    def __init__(self, config, feature_dim):
        nn.Module.__init__(self)
        self.mlp1 = nn.Linear(feature_dim, 128)
        self.bn1 = nn.BatchNorm1d(128)
        self.relu1 = nn.LeakyReLU(128)
        self.mlp2 = nn.Linear(128 , 256)
        self.bn2 = nn.BatchNorm1d(256)
        self.relu2 = nn.LeakyReLU(256)

        self.mlp3 = nn.Linear(256 , 128)
        self.bn3 = nn.BatchNorm1d(128)
        self.relu3 = nn.LeakyReLU(128)


        self.mlp4 = nn.Linear(128 , config['class_num'])
        self.bn4 = nn.BatchNorm1d(config['class_num'])
        self.relu4 = nn.LeakyReLU(config['class_num'])


    def forward(self, feature,cluster_index):
        y = self.bn2(self.relu2(self.mlp2(self.relu1(self.bn1(self.mlp1(feature))))))
        [region_index, region_mask] = np.unique(cluster_index.cpu().numpy(), False, True)
        region_mask = torch.from_numpy(region_mask).cuda()
        global_feature, argmax = scatter_max(y, region_mask, dim = 0)
        semantic = self.bn4(self.relu4(self.mlp4(self.relu3(self.bn3(self.mlp3(global_feature))))))
        return semantic
