...
RuleBook &Metadata<dimension>::getSubmanifoldRuleBook(
  ...
  if (rb.empty()) {
    ...
    {
        SubmanifoldConvolution_SgsToRules(SGs, rb, size.data<long>(),n, dilated_rate);
    }
  }
  return rb;
}
...
RBChunkPointerList & Metadata<dimension>::getSubmanifoldChunkRuleBook(
  ...
  if (chunkPointerList.list.empty()) {
    ...
    {
        SubmanifoldConvolution_SgsToRules(SGs, chunkPointerList, size.data<long>(),n, dilated_rate);
    }
#ifdef PRINT_MEM_ALLOC
